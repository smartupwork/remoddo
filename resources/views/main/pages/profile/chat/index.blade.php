                            ...
                            <form class="input-search-form w-100">
                                <label class="relative">
                                    <input type="text" class="input-search-window w-100 search-chat"
                                           data-search-url="{{route('main.profile.chat.search',['order'=>request()->order])}}"
                                ...
                                @if($chat->last_message)
                                    <a href="{{route('main.profile.chat.edit',['order'=>$chat->order_id])}}"
                                       class="messages-chatlist__item to-messages
                                        @if(request()->order->id==$chat->order_id) active @endif
                                        @if(request()->order->id!=$chat->order_id && $chat->not_read_message_count>0 && $chat->last_message->recipient_id==auth()->id())notification @endif" data-messages="{{$chat->not_read_message_count}}">
                                    ...
                                    <a href="{{route('main.profile.chat.edit',['order'=>$chat->order_id])}}"
                                       class="messages-chatlist__item to-messages ">
                                        <div class="d-flex flex-column justify-content-between flex-auto">
                                            <div class="d-flex justify-content-between">
                                            <span
                                                class="messages-chatlist__name">{{auth()->user()->info->full_name}}</span>
                                            </div>
                                            <p class="messages-chatlist__message ellipsis"></p>
                                        </div>
                                    </a>
                                @endif
                            @endforeach
