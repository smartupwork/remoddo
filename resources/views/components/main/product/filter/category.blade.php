...
<div class="pb-4">
    ...
    @if (request()->has('category'))
        @php
            $selectedCategoryIds = request()->get('category');
            $urlSlug = request()->segment(2); // Get the category slug from the URL
        @endphp

        @foreach ($categories->sortBy('title') as $category)
            @php
                $isSelected = in_array($category->id, $selectedCategoryIds);
                $hasSlug = $urlSlug === $category->slug; // Compare URL slug with category slug
            @endphp

            @if (!$hasSlug && ($isSelected || $hasSlug))
                <label class="custom-checkbox mb-4">
                    <input name="category[]" {{ $isSelected ? 'checked' : '' }} value="{{ $category->id }}"
                        class="custom-checkbox__input" type="checkbox">
    ...
    <label class="custom-checkbox mb-4">
        ...
        @foreach ($categories->sortBy('title')->slice(0, 9) as $category)
            <label class="custom-checkbox mb-4">
                <input name="category[]" @if (
                    (request()->has('category') && in_array($category->id, request()->get('category'))) ||
                        (isset(request()->category) && !is_array(request()->category) && request()->category->slug === $category->slug)) checked @endif value="{{ $category->id }}"
                    class="custom-checkbox__input" type="checkbox">
                <span class="custom-checkbox__input-fake "></span>
                <span class="custom-checkbox__label">{{ $category->title }} ({{ $category->products_count }})</span>
            </label>
